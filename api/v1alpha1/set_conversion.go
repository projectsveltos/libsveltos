...
func (src *Set) ConvertTo(dstRaw conversion.Hub) error {
	dst := dstRaw.(*libsveltosv1beta1.Set)

	configlog.V(logs.LogInfo).Info("convert Set from v1alpha1 to v1beta1")
...
func (dst *Set) ConvertFrom(srcRaw conversion.Hub) error {
	src := srcRaw.(*libsveltosv1beta1.Set)

	configlog.V(logs.LogInfo).Info("convert Set from v1beta1 to v1alpha1")
