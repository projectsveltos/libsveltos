        ...
        properties:
          ...
          spec:
              ...
              workloadIdentity:
                ...
                properties:
                  ...
                  caSecretRef:
                    ...
                    x-kubernetes-map-type: atomic
                  endpoint:
                    description: Endpoint is the API server endpoint of the managed
                      cluster (e.g. https://…).
                    minLength: 1
                    type: string
                  gcp:
                    ...
                    properties:
                      clusterName:
                        description: ClusterName is the name of the GKE cluster.
                        minLength: 1
                        type: string
                      location:
                        description: Location is the GCP region or zone of the GKE
                          cluster (e.g. "us-central1").
                        minLength: 1
                        type: string
                      projectID:
                        description: ProjectID is the GCP project ID that contains
                          the GKE cluster.
                        minLength: 1
                        type: string
                    required:
                ...
                type: object
                x-kubernetes-validations:
                - message: aws must be set if and only if provider is AWS
                  rule: (self.provider == 'AWS') == has(self.aws)
                - message: gcp must be set if and only if provider is GCP
            ...
            - message: 'workloadIdentity, kubeconfigName: conflict'
              rule: '!has(self.workloadIdentity) || !has(self.kubeconfigName)'
          status:
